...
public interface CardStatusRepository extends JpaRepository<CardStatus, Long> {
    @Modifying
    @Transactional
    @Query(value="DROP TABLE IF EXISTS issuingbankschema.card_status CASCADE", nativeQuery=true)
    void dropTable();

    @Modifying
    @Transactional
    @Query(value="CREATE TABLE IF NOT EXISTS issuingbankschema.card_status(" +
    ...
    @Modifying
    @Transactional
    @Query(value="INSERT INTO issuingbankschema.card_status(card_status_name)" +
